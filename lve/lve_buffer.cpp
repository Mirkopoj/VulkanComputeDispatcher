...
/**
 * Copies the mapped buffer to the specified data . Default value writes
 * whole buffer range
 *
 * @param data Pointer to the data's destination.
 * @param size (Optional) Size of the data to copy. Pass VK_WHOLE_SIZE to
 * flush the complete buffer range.
 * @param offset (Optional) Byte offset from beginning of mapped region
 *
 */
void LveBuffer::readFromBuffer(void *data, VkDeviceSize size,
                              VkDeviceSize offset) {
   assert(mapped && "Cannot copy to unmapped buffer");
